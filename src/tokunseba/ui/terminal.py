...
EVENT_TONE: dict[str, str] = {
    ...
    "record_error": "bad", "passthrough_after_error": "bad",
}
...
# --------------------------------------------------------------------------- renderables
def stat_tiles(stats: dict) -> Table:
    """A borderless row of tiles: the five numbers that answer 'is this worth it'."""
    s = stats or {}
    accent, dim, good = style("accent"), style("dim"), style("good")
    tiles = [
        ("tokens saved", k(s.get("tokens_saved")), f"{pct(s.get('pct_saved'))} of tool output", good),
        ("money saved", usd(s.get("usd_saved")), "versus no tokunseba", good),
        ("spent", usd(s.get("usd_spent")), f"{k(s.get('input_tokens'))} in / {k(s.get('output_tokens'))} out", accent),
        ("cache hit rate", pct(s.get("cache_hit_rate")), f"{k(s.get('cache_read'))} read from cache", accent),
        ("requests", k(s.get("requests")), f"{len(s.get('by_tool') or [])} tools", accent),
    ]
...
def sparkline(daily: list[dict], width: int = 48) -> str:
    """Tokens saved per day as unicode blocks, always exactly `width` characters."""
    rows = list(daily or [])
    width = max(1, int(width))
    if not rows:
        return f"[{style('dim')}]no data yet[/]"
    values = [max(0, int(r.get("tokens_saved") or 0)) for r in rows]
    n = len(values)
    buckets: list[int] = []
    ...
    out = []
    for v in buckets:
        out.append(BLOCKS[0] if v <= 0 else BLOCKS[max(1, round(v / top * (len(BLOCKS) - 1)))])
    return "".join(out)
...
def by_tool_table(rows: list[dict]) -> Table:
    ...
    for r in rows:
        ...
        bar = BAR * max(1, round(saved / top * 20)) if saved > 0 else ""
