...
def _rec(**kw):
    base = dict(id="r1", ts=time.time(), session_id="s1", tool_id="claude-code", project="/p",
                ...
                est_tokens_after=6000, cost_usd=0.02, counterfactual_usd=0.03, arm="control",
...
def test_project_filter_and_spend(home):
    led = Ledger(home / "ledger.sqlite")
    ...
    assert round(led.spend_since(0), 4) == 0.04
    assert round(led.spend_since(0, "/one"), 4) == 0.02
